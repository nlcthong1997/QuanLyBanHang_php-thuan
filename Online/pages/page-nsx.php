<?php  
	if(!isset($_GET['idnsx']) || $_GET['idnsx'] == ''){
?>
	<div>
		<center>
...
<?php
		...
		$sql = "SELECT * FROM nsx WHERE MaNSX=$idnsx";
		$rs = load($sql);
		$row = $rs->fetch_assoc();
		// ------------------------------------
		...
		$sql_total = "SELECT COUNT(*) AS total FROM sanpham WHERE MaNSX=$idnsx";
		...
		// -------------------------------------
		$sql_sp = "SELECT * FROM sanpham WHERE MaNSX=$idnsx LIMIT $of, $limit";
		$rs_sp = load($sql_sp);
?>
	<div class="features_items"><!--features_items-->
		<h2 class="title text-center">MÁY ẢNH <?= $row['TenNSX'] ?></h2>
		<?php  
	...
	<div>
		...
		<ul class="pagination">
			<?php if($prev_page > 0): ?>
		  		<li><a href="index.php?Page=nsx&idnsx=<?= $idnsx ?>&idpage=<?= $prev_page ?>"> < </a></li>
			<?php endif; ?>
		  	<?php for ($i=1; $i <= $num_pages; $i++) : ?>
			  	<li class="<?php if($i == $current_page) echo 'active'?>">
			  		<a href="index.php?Page=nsx&idnsx=<?= $idnsx ?>&idpage=<?= $i ?>"> <?= $i ?></a>
			  	</li>
			<?php endfor; ?>
			<?php if($next_page <= $num_pages): ?>
		  		<li><a href="index.php?Page=nsx&idnsx=<?= $idnsx ?>&idpage=<?= $next_page ?>"> > </a></li>
			<?php endif; ?>
		</ul>
		</center>
	</div>
