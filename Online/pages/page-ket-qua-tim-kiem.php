...
<?php
		...
		$sql_total = "SELECT COUNT(*) AS total FROM sanpham WHERE TenSP LIKE '%$tukhoa%'";
		...
		$sql = "SELECT * FROM sanpham WHERE TenSP LIKE '%$tukhoa%' LIMIT $of, $limit";
		$rs = load($sql);
?>
	...
	<div>
		...
		<ul class="pagination">
			<?php if($prev_page > 0): ?>
		  		<li><a href="index.php?Page=tim-kiem&id=<?= $tukhoa ?>&idpage=<?= $prev_page ?>"> < </a></li>
			<?php endif; ?>
		  	<?php for ($i=1; $i <= $num_pages; $i++) : ?>
			  	<li class="<?php if($i == $current_page) echo 'active'?>">
			  		<a href="index.php?Page=tim-kiem&id=<?= $tukhoa ?>&idpage=<?= $i ?>"> <?= $i ?></a>
			  	</li>
			<?php endfor; ?>
			<?php if($next_page <= $num_pages): ?>
		  		<li><a href="index.php?Page=tim-kiem&id=<?= $tukhoa ?>&idpage=<?= $next_page ?>"> > </a></li>
			<?php endif; ?>
		</ul>
		</center>
	</div>
